...
/* ---------------- 彭博ECVO式：分项贡献热图表（共享，六之二分区复用） ---------------- */
var MM3 = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
function mLabel(p) { return MM3[+p.slice(5, 7) - 1] + '-' + p.slice(2, 4); }
function mapOf2(arr) { var m = {}; (arr || []).forEach(function (x) { m[x[0]] = x[1]; }); return m; }
function heatCell(v) {
  if (v == null) return '<td style="color:#56607a;text-align:right">—</td>';
  var a = Math.min(0.12 + Math.abs(v) / 0.45 * 0.6, 0.72);
  var bg = v >= 0 ? ('rgba(229,115,74,' + a.toFixed(2) + ')') : ('rgba(91,155,213,' + a.toFixed(2) + ')');
  return '<td style="background:' + bg + ';color:#f0f3f9;text-align:right">' + (v >= 0 ? '+' : '') + v.toFixed(3) + '</td>';
}
/* panel=release_panels.json整体; nM=展示最近几个月。行: 所有项目/核心(参考)/核心服务+八明细/核心商品/食品/能源 */
window.cpiBloombergTable = function (P, nM) {
  var B = (P && P.blocks) || {}, hist = B.cpi_hist, det = B.cpi_detail;
  if (!hist || !hist.groups || !(hist.headline || []).length) return '';
  var months = hist.headline.map(function (x) { return x[0]; }).slice(-(nM || 9));
  var hl = mapOf2(hist.headline), cr = mapOf2(hist.core);
  var G = {};
  Object.keys(hist.groups).forEach(function (g) { G[g] = { w: hist.groups[g].w, m: mapOf2(hist.groups[g].mom) }; });
  function gC(g, mo) { var o = G[g]; if (!o) return null; var v = o.m[mo]; return v == null ? null : +(o.w * v / 100).toFixed(3); }
  var h = '<div style="overflow-x:auto;margin-top:8px;"><table style="font-size:11.5px;white-space:nowrap;border-collapse:collapse;">';
  h += '<tr><th style="text-align:left;">系列</th>' + months.map(function (mo) { return '<th style="text-align:right">' + mLabel(mo) + '</th>'; }).join('') + '<th style="color:#d4a944;text-align:right">权重</th></tr>';
  function row(name, fn, w, bold, indent, dim) {
    var tds = months.map(function (mo) { return heatCell(fn(mo)); }).join('');
    return '<tr style="' + (dim ? 'opacity:.72;' : '') + '"><td style="text-align:left;' + (bold ? 'font-weight:700;color:#e8c34a;' : '') + '">' +
      (indent ? '&nbsp;&nbsp;├ ' : '') + name + '</td>' + tds +
      '<td style="color:#d4a944;text-align:right">' + (w != null ? w.toFixed(3) : '') + '</td></tr>';
  }
  h += row('所有项目', function (mo) { return hl[mo] != null ? +hl[mo].toFixed(3) : null; }, null, true, false, false);
  h += row('核心CPI（参考=核心环比）', function (mo) { return cr[mo] != null ? +cr[mo].toFixed(3) : null; }, null, false, false, true);
  h += row('核心服务', function (mo) { return gC('核心服务', mo); }, 60.272, true, false, false);
  ((det && det.items) || []).forEach(function (it) {
    var mm = mapOf2(it.mom);
    h += row(it.name, function (mo) { var v = mm[mo]; return v == null ? null : +(it.w * v / 100).toFixed(3); }, it.w, false, true, false);
  });
  h += row('核心商品', function (mo) { return gC('核心商品', mo); }, 18.842, true, false, false);
  h += row('食品', function (mo) { return gC('食品', mo); }, 13.540, true, false, false);
  h += row('能源', function (mo) { return gC('能源', mo); }, 7.347, true, false, false);
  h += '</table></div><p style="font-size:10.5px;color:var(--dim);margin:4px 0 0;">单元格=环比贡献(pp)≈权重×分项环比/100 · 橙=推升通胀 蓝=拖累，色深=幅度 · 权重=BLS相对重要性2025-12快照（与彭博一致）' + (det && det.src ? ' · ' + esc(det.src) : '') + '</p>';
  return h;
};
window.cpiStackChart = cpiHistChart; /* 供六之二分区复用 */
  ...
  order.forEach(function (o) {
    if (o.k === 'cpi') {
      ...
      cards.push(card('CPI · 消费者价格', o.b.period, o.fr,
        ...
        (window.cpiBloombergTable ? window.cpiBloombergTable(P, 9) : ''),
...
window.renderBig4Panel = function (hostId, src) {
  ...
  Promise.all([
    ...
    host.innerHTML = '<div class="card"><p style="color:var(--dim);font-size:13px;margin:0;">四大发布数据加载失败（' + e + '），请刷新页面重试。</p></div>';
  });
};
})();

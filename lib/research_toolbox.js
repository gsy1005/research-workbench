...
window.researchToolbox=function(hostId){
  ...
  let sel=[], mode='line', rng='3y', logy=false, smooth=false, fxN=0, lag=0;
  let comboSel='';
  host.innerHTML=`
  ...
  function renderCorr(st,tip,ex){
    ...
    /* 频率对齐: 日度×月度混选时, 日度序列降采样为月末值, 统一按月键对齐(否则共同日期太少) */
    const raw=sel.map(id=>(HIST[id]||[]).filter(p=>!st||p[0]>=st));
    const gapOf=s=>{if(s.length<10)return 99;const ds=[];
      for(let i=1;i<Math.min(s.length,80);i++)ds.push((Date.parse(s[i][0])-Date.parse(s[i-1][0]))/864e5);
      ds.sort((a,b)=>a-b);return ds[Math.floor(ds.length/2)]||99;};
    const gaps=raw.map(gapOf),mxG=Math.max.apply(null,gaps);
    const norm=raw.map((s,k)=>{
      if(mxG>20&&gaps[k]<=7){const mm={};s.forEach(p=>{mm[p[0].slice(0,7)]=p[1];});
        return Object.keys(mm).sort().map(k2=>[k2,mm[k2]]);}
      if(mxG>20&&gaps[k]>20){return s.map(p=>[p[0].slice(0,7),p[1]]);}
      return s;});
    /* 日变化: %单位取差分, 其余取变动率 */
    const chg=norm.map((s,k)=>{
      const u=catOf(sel[k]).unit;
      const out=[];
    ...
    /* 滞后期: corr(A_t, B_{t+L}), L>0 = A领先L期(如"M2领先资产3个月"型信号验证) */
    const L=Math.max(0,Math.min(lag,Math.floor(common.length/4)));
    const rl=(ci,cj)=>{const n=common.length-L;return n<20?null:pearson(ci.slice(0,n),cj.slice(L));};
    let h='<div style="margin:2px 0 8px">滞后期：<span class="seg" id="rt-lag">'
      +[0,1,2,3,6,12].map(v=>`<button data-l="${v}" class="${v===L?'on':''}">${v===0?'0(同期)':v+'期'}</button>`).join('')
      +'</span> <span class="hint">验证"A领先B N个月"型信号：选滞后N，看首行与其余列；月度×日度按共同日期对齐≈月度</span></div>';
    h+='<table><tr><th>相关系数(日变化'+(L?`,滞后${L}期`:'')+')</th>'+sel.map((id,i)=>`<th>${LET[i]} ${catOf(id).name}</th>`).join('')+'</tr>';
    cols.forEach((ci,i)=>{
      h+=`<tr><td><b style="color:${PAL[i%8]}">${LET[i]}</b> ${catOf(sel[i]).name}</td>`;
      cols.forEach((cj,j)=>{
        const r=i===j?1:rl(ci,cj);
      ...
      h+='</tr>';});
    ex.innerHTML=h+'</table><div class="hint">样本=所选区间内共同交易日变化（%单位序列取差分，其余取变动率）；滞后L期= corr(X_t, Y_{t+L}) 即行领先列；|r|≥0.7 金色加粗</div>';
    ex.querySelectorAll('#rt-lag button').forEach(b=>b.onclick=()=>{lag=+b.dataset.l;render();});
    /* 滚动63日相关(前两序列, 同滞后) */
    const a=cols[0].slice(0,common.length-L),b2=cols[1].slice(L),dts=common.slice(L);
    const roll=[];const W=63;
    for(let i=W;i<a.length;i++){
      const x=a.slice(i-W,i),y=b2.slice(i-W,i);
      const r=pearson(x,y);if(r!=null)roll.push([dts[i],+r.toFixed(3)]);}
    chart.clear();
    chart.setOption({backgroundColor:'transparent',tooltip:tip,
      ...
      series:[{name:`滚动63日相关 ${LET[0]}×${LET[1]}${L?`(A领先${L}期)`:''}`,type:'line',showSymbol:false,data:roll,

...
+ '.ec-note{font-size:11px;color:var(--muted,#8b95a5);margin-top:8px;line-height:1.6}';
  ...
  /* renderEconCal(hostId, ecJson, opts): 按 days[].grp 分组为按钮页签；周日默认“下周”，其他日期默认“本周”，opts.btnHostId 可把按钮条挪到标题旁 */
  window.renderEconCal = function(hostId, ec, opts){
    ...
    /* 以北京时间判断：周日打开页签时直接落在“下周”；即使当天管线尚未重跑，也不会继续停在本周一/周二。 */
    var bjtDay = new Date(Date.now() + 8 * 3600 * 1000).getUTCDay();
    var wanted = bjtDay === 0 ? '下周' : (ec.default_grp || '本周');
    var defIdx = grps.indexOf(wanted);
    if(defIdx < 0) defIdx = grps.indexOf(bjtDay === 0 ? '下周' : '本周');
    if(defIdx < 0) defIdx = 0;
    var h = '<div class="ec-btnbar">' + grps.map(function(g,i){
      return '<button class="ec-btn'+(i===defIdx?' on':'')+'" data-ecp="'+i+'">'+g+'</button>';
    }).join('') + '</div>';
    h += grps.map(function(g,i){
      return '<div class="ec-panel'+(i===defIdx?' on':'')+'" data-ecp="'+i+'"><div class="ec-grid">'

...
+ '.ec-btnbar{display:flex;gap:10px;margin-top:10px;flex-wrap:wrap}'
+ '.ec-titlebtns{float:right}'
+ '.ec-titlebtns .ec-btnbar{margin-top:0;display:inline-flex;gap:8px}'
...
+ '.ec-panel{margin-top:10px;display:none}'
...
+ '.ec-note{font-size:11px;color:var(--muted,#8b95a5);margin-top:8px;line-height:1.6}';
  ...
  /* renderEconCal(hostId, ecJson, opts): 默认每天只平铺星级最高的1条(并列取时间最早),其余全部折叠「其他+N」 */
  window.renderEconCal = function(hostId, ec, opts){
    ...
    var direct = ec.days.filter(function(d){ return !d.collapsed; });
    var folded = ec.days.filter(function(d){ return d.collapsed; });
    var h = '<div class="ec-grid">' + direct.map(dayCard).join('') + '</div>';
    /* collapsed 分组:按钮切换,点了才显示(内容同样只留最重要一条) */
    if(folded.length){
      h += '<div class="ec-btnbar">' + folded.map(function(d,i){
        return '<button class="ec-btn" data-ecp="'+i+'">'+d.date+' ▸</button>';
      }).join('') + '</div>';
      h += folded.map(function(d,i){
        return '<div class="ec-panel" data-ecp="'+i+'">'+dayCard(d)+'</div>';
      }).join('');
    }
    ...
    if(bar){
      bar.querySelectorAll('.ec-btn').forEach(function(btn){
        btn.onclick = function(){
          ...
          var panel = host.querySelector('.ec-panel[data-ecp="'+id+'"]');
          var wasOn = btn.classList.contains('on');
          bar.querySelectorAll('.ec-btn').forEach(function(b){ b.classList.remove('on'); });
          host.querySelectorAll('.ec-panel').forEach(function(p){ p.classList.remove('on'); });
          if(!wasOn){ btn.classList.add('on'); if(panel) panel.classList.add('on'); }
        };
      });
    }
  };

...
(function () {
  ...
  function renderNfp(b) {
    ...
    h += '<div style="margin-top:6px">' + barTable(rows, 'chg', mx, '', false).replace(/<td style="width:12%">([\s\S]*?)<\/td>/g, function (m, g) { return '<td style="width:12%">' + g.trim() + '</td>'; }) + '</div>';
  ...
  function renderRetail(b) {
    ...
    h += '<div style="margin-top:6px">' + barTable(rows, 'mom', mx, '%', false) + '</div>';
  ...
  function renderCpi(b) {
    ...
    h += '<div style="margin-top:6px">' + barTable(rows, 'contrib', mx, '', true) + '</div>';

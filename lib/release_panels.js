/* 数据发布·分项贡献面板 — 非农/CPI/PCE/GDP/FedWatch
   数据: data/release_panels.json (scripts/fetch_release.py 管线产出, 全部L1官方源)
...
(function () {
  ...
  function renderNfp(b) {
    ...
    var h = '<div class="rp-card"><div class="rp-t">非农就业分项<span class="per">' + b.period + '</span></div>' + srcLine(b);
    h += '<div class="rp-chips"><span class="chip">总变动 <b>' + fmt(b.total_chg, '千人', 0) + '</b></span></div>';
    h += barTable(rows, 'chg', mx, '', false).replace(/<td style="width:12%">([\s\S]*?)<\/td>/g, function (m, g) { return '<td style="width:12%">' + g.trim() + '</td>'; });
  ...
  function renderCpi(b) {
    ...
    var h = '<div class="rp-card"><div class="rp-t">CPI分项 · 同比贡献<span class="per">' + b.period + '</span></div>' + srcLine(b);
    ...
    h += barTable(rows, 'contrib', mx, '', true);
    return h + '<div class="rp-src" style="margin-top:4px">同比贡献(pp)≈权重×分项同比/100 · *=停摆缺月为跨期环比</div></div>';
  }
  function renderBea(key, title, dec) {
    var b = P.blocks[key]; if (!b || b.empty) return '';
